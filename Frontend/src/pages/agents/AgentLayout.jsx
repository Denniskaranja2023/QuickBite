...
import { Home, Star, UtensilsCrossed, LogOut, Menu, X, ChevronDown, User } from 'lucide-react';

function AgentLayout({ user, onLogout }) {
  const location = useLocation();
  const [menuOpen, setMenuOpen] = useState(false);

  const navItems = [
    { path: '/agent/dashboard', icon: Home, label: 'Dashboard' },
    { path: '/agent/reviews', icon: Star, label: 'Reviews' },
  ];
        ...
        <div className="p-4 border-b border-gray-100">
          <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 capitalize">{user?.user_type || 'Agent'}</p>
            </div>
          </div>
        </div>
